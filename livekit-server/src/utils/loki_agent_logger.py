"""
Loki Logger Configuration for LiveKit Agent (main.py)
Separate logger for agent logs with label: app='livekit-agent'
"""
import logging
import os
...
def setup_agent_logger():
    """
    Configure and return logger for LiveKit Agent with console, file, and Loki handlers
    """
    # Create logger
    ...
    try:
        ...
        file_handler = RotatingFileHandler(
            filename=os.path.join(log_dir, "livekit_agent.log"),
        ...
        print(f"✅ [LOGGING] File handler configured: {log_dir}/livekit_agent.log")
    ...
    if loki_host and loki_user and loki_password:
        try:
            ...
            print("🔧 [LOKI] Initializing Loki handler for LiveKit Agent...")
            
            loki_handler = LokiHandler(
                url=f"{loki_host}/loki/api/v1/push",
                tags={"app": "livekit-agent", "environment": "production"},
            ...
            print(f"✅ [LOKI] Handler configured for LiveKit Agent. Sending logs to: {loki_host}")
            print(f"✅ [LOKI] Label: app='livekit-agent'")
